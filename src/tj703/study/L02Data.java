...
java는 숫자, 유니코드문자, 불리언만 기본데이터.

    그리고 다시 숫자를 정수와 실수로 구분.
    또 다시 정수와 실수를 숫자의 크기의 따라 구분함
    정수는 byte, short, int, long으로 구분. (각 1,2,4,8바이트)
    실수는 float (4바이트)와 double (8바이트)로 구분

js는 동적 타입의 언어로 숫자, 문자열, 불리언 외에도
undefined, null, Symbol, BigInt을 기본데이터로 제공
     */