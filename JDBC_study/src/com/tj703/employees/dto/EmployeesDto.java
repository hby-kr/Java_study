...
// DAO를 별개의 클래스로 만듬으로써 데이터 접근 로직과 비즈니스 로직을 분리하여
// 코드의 유지보수성과 재사용성을 향상시킬 수 있습니다.
...
// ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
    // SalariesDto 따로 만들었고, 그것을 employeeDto 안에 넣으려고 하기 있으므로,
    // employeeDto에 SalariesDto가 들어갈 수 있는 자리를 만들어주기

    // emp : sal = 1 : n   join 임. 사원이 복수의 급여정보를 가지고 있다.

    // 필드 선언
    //1:n인데 1:1인거처럼 출력
    private SalariesDto salaries;  // 말그대로 SalariesDto
    private List<SalariesDto> salariesList; // SalariesDto 객체를 넣을 List
    // 하나의 사람 정보에, 여러 봉급값을 넣을려고 하는 것. list 안에 다시 list 넣는 것.

    // 필드에 대한 getter,setter 매서드 구현
